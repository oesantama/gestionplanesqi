...
const WARNING_DAYS = 5; // Aviso previo a 5 días

/**
 * Valida complejidad de la contraseña (ISO 27001 / BASC)
 */
...
function calculatePasswordExpirationInfo(passwordActualizadoEn) {
  ...
  const diasParaVencer = PASSWORD_EXPIRATION_DAYS - diasTranscurridos;

  return {
    dias_transcurridos: diasTranscurridos,
    dias_para_vencer: diasParaVencer,
    password_expirado: diasParaVencer <= 0,
    password_por_vencer: diasParaVencer <= WARNING_DAYS && diasParaVencer > 0
  };
}
...
async function checkPasswordReuseHistory(usuarioId, newPassword) {
  ...
  for (const row of historyRows) {
    ...
    if (match) {
      const err = new Error("⚠️ Por políticas de seguridad (ISO 27001 / BASC), no puedes reutilizar ninguna de tus últimas 3 contraseñas anteriores.");
      err.status = 400;
      throw err;
    }
  }
